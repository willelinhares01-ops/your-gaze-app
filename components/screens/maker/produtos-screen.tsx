...
type Produto = {
  ...
  loja: string
  url: string
  // mídia opcional daquele produto: foto ou vídeo curto (até 10s)
  midiaUrl?: string
...
export function ProdutosScreen() {
  ...
  const escolherMidia = (file: File) => {
    ...
    if (file.type.startsWith('video/')) {
      // valida a duração: vídeo do produto pode ter no máximo 10 segundos
      const v = document.createElement('video')
      v.preload = 'metadata'
      v.onloadedmetadata = () => {
        if (v.duration > 10.5) {
          URL.revokeObjectURL(url)
          setErro('O vídeo do produto deve ter no máximo 10 segundos.')
          return
        }
        setMidiaUrl(url)
        setMidiaTipo('video')
      ...
      setErro('Selecione uma imagem ou um vídeo de até 10 segundos.')
    }
  }
  ...
  return (
    <div className="px-6 py-8">
      <header className="mb-6">
        <h2 className="font-heading text-2xl text-navy">Minha Lista de Produtos</h2>
        <p className="text-sm text-muted-foreground">
          Divulgue produtos de qualquer loja que você vender — o lucro é 100% seu.
        </p>
      </header>

      <div className="mb-6 rounded-xl border border-border bg-card p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
          {/* Espaço para foto ou vídeo de 10s do produto */}
            ...
            {midiaUrl ? (
              <div className="relative size-24 overflow-hidden rounded-lg border border-border">
                {midiaTipo === 'video' ? (
                  ...
                  <img src={midiaUrl || '/placeholder.svg'} alt="Prévia do produto" className="size-full object-cover" />
                )}
                <button
                  onClick={limparMidia}
              ...
              >
                ...
                <span className="text-[10px] leading-tight">Foto ou vídeo 10s</span>
              </button>
            )}
          </div>
          ...
          <div className="flex flex-1 flex-col gap-2 sm:flex-row">
            ...
            <input
              ...
              placeholder="Loja / link (qualquer uma)"
              className="flex-1 rounded-lg border border-input bg-background px-3 py-2.5 text-sm outline-none focus:border-gold"
            />
            <button
              onClick={add}
      ...
      <ul className="grid gap-4 sm:grid-cols-2">
        {produtos.map((p) => (
          <li key={p.id} className="overflow-hidden rounded-xl border border-border bg-card">
            {/* Mídia do produto no topo (foto ou vídeo de 10s) */}
            <div className="relative aspect-video w-full bg-secondary">
              {p.midiaUrl ? (
                p.midiaTipo === 'video' ? (
                  ...
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={p.midiaUrl || '/placeholder.svg'} alt={p.nome} className="size-full object-cover" />
                )
              ) : (
            ...
            {/* Link do produto logo abaixo da mídia */}
            <div className="flex items-center justify-between p-4">
              ...
              <a
                ...
                aria-label={`Abrir link de ${p.nome}`}
              >
